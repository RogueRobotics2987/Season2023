...
void AutoBalance::Execute() {
  ...
  while(m_state == 0) {
    // m_angle = frc::SmartDashboard::GetNumber("m_angle value: ", m_angle);
      ...
      frc::SmartDashboard::PutNumber("Auto Pitch", m_drive->GetPitch());
      frc::SmartDashboard::PutNumber("Auto Timer", m_timer.Get().value());
      m_drive->Drive(0.8_mps, 0_mps, 0_rad_per_s, false, false);
    ...
    while(m_state == 1){
      ...
      frc::SmartDashboard::PutNumber("Auto Pitch", m_drive->GetPitch());
      frc::SmartDashboard::PutNumber("Auto Timer", m_timer.Get().value());
      m_drive->Drive(0.35_mps, 0_mps, 0_rad_per_s, false, false);
    ...
    while(m_state == 2) {
      ...
      if(m_drive->GetPitch() <= 8 && backwardsCheck == false) {
        m_state = 3;
        m_timer.Start();
        backwardsCheck = true;
      }
      else if(m_drive->GetPitch() < 1 && m_drive->GetPitch() > -1 && m_timer.Get() > 5.0_s) {
          m_state = 5;
        }
      ...
      while(m_state == 4) {
        ...
        frc::SmartDashboard::PutNumber("Auto Pitch", m_drive->GetPitch());
        frc::SmartDashboard::PutNumber("Auto Timer", m_timer.Get().value());
        if(m_drive->GetPitch() >= 1) {
          m_drive->Drive(-0.15_mps, 0_mps, 0_rad_per_s, false, false);
        }
        else if(m_drive->GetPitch() <= -1) {
          m_drive->Drive(0.15_mps, 0_mps, 0_rad_per_s, false, false);
        }
        else if(m_drive->GetPitch() < 1 && m_drive->GetPitch() > -1) {
          m_state = 2;
